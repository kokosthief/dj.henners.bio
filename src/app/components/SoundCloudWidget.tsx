...
const SoundCloudWidget: React.FC = () => {
  ...
  return (
    <section id="listen" className="mx-auto w-full max-w-6xl px-5 py-8 sm:px-6 lg:px-8">
      <div className="rounded-[2rem] border border-amber-200/15 bg-white/[0.06] p-6 shadow-2xl shadow-cyan-950/20 backdrop-blur sm:p-8">
        ...
        {!loaded ? (
          <div className="flex flex-col gap-4 rounded-3xl border border-white/10 bg-[#0b1220] p-5 sm:flex-row sm:items-center sm:justify-between sm:p-6">
            <div>
              <h3 className="text-lg font-semibold text-white">{track.title}</h3>
              <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-400">
                Player kept unloaded until you click, so the page stays fast and does not leave a dead blank embed area.
              </p>
            </div>
